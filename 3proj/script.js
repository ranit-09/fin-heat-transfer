function calculate() {
  ...
  if (
    ...
    alert(
      "Error: Number of xLength and temperature values must equal inputPoints."
    );
    return;
  }
  ...
  for (let i = 0; i < inputPoints; i++) {
    if (temperatureValues[i] <= ambientTemperature) {
      alert(
        "Error: Temperature must be greater than ambient temperature at all points."
      );
      return;
    }
  ...
  const m_neg =
    (inputPoints * sumXY - sumX * sumY) / (inputPoints * sumX2 - sumX * sumX);
